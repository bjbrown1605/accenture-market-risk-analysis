...
# Função para buscar os dados com cache para evitar downloads repetidos
@st.cache_data
...
if st.sidebar.button('Executar Análise'):
    ...
    try:
        ...
        # Usando colunas para organizar os cartões
        cols = st.columns(len(tickers))
        for i, ticker in enumerate(tickers):
            with cols[i]:
                st.metric(
                    label=f"Volatilidade Anualizada ({ticker})",
                    value=f"{volatilidade[ticker]:.2%}"
                )
                st.metric(
                    label=f"VaR Diário 95% ({ticker})",
                    value=f"{var[ticker]:.2%}"
                )
        
        # --- 3.2. Análise de Desempenho ---
        st.header('Análise de Desempenho')
        
        # Gráfico de Preços com Eixo Duplo
        fig_precos = make_subplots(specs=[[{"secondary_y": True}]])
        fig_precos.add_trace(go.Scatter(x=precos.index, y=precos[tickers[0]], name=f'{tickers[0]} (R$)'), secondary_y=False)
        fig_precos.add_trace(go.Scatter(x=precos.index, y=precos[tickers[1]], name=f'{tickers[1]} (Pontos)'), secondary_y=True)
        fig_precos.update_layout(title_text='Série Histórica de Preços')
        fig_precos.update_yaxes(title_text=f'Preço {tickers[0]} (R$)', secondary_y=False)
        fig_precos.update_yaxes(title_text=f'Pontuação {tickers[1]}', secondary_y=True)
        ...
        with col1:
            ...
            fig_heatmap = px.imshow(correlacao, text_auto=True, title='Heatmap de Correlação')
            st.plotly_chart(fig_heatmap, use_container_width=True)
        with col2:
            # Histograma de Retornos
            fig_hist = px.histogram(retornos, marginal='box', title='Distribuição dos Retornos Diários')
